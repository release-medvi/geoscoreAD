...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        out = self.transformer(x, features_list, DPAM_layer=DPAM_layer, ffn=ffn)
...
class AnomalyCLIP(nn.Module):
    def __init__(self,
        ...
        self.num_pca_components = -1
        self.pca = None
        self.patch_size=vision_patch_size
        '''

    def get_text_features(self, image=None, cls_name=None):
    ...
    ## added for DPMM comparison
    ## this is for single layer feature extraction
    def extract_features_single(self, image: torch.Tensor):
        # input image shape: NCHW [12, 3, 448, 448]
        features_list = [24] # [6, 12, 18, 24]
        image = image.to(next(self.parameters()).device, dtype=self.dtype)
        #print("Image device:", image.device, "dtype:", image.dtype, image.shape)
        #print("Model device:", next(self.parameters()).device, "dtype:", next(self.parameters()).dtype)
        image_features, patch_features = self.encode_image(image, features_list, DPAM_layer = 20)
        patch_feature = patch_features[-1]  # use the last layer features
        patch_feature = patch_feature/ patch_feature.norm(dim = -1, keepdim = True)   

        return patch_feature[:,1:] # [B, N, D], remove cls token

    ## added for DPMM comparison
    ''' 
    # this is for multi-layer feature extraction
    def extract_features(self, image: torch.Tensor):
        ...
        #print("Model device:", next(self.parameters()).device, "dtype:", next(self.parameters()).dtype)
        image_features, patch_features = self.encode_image(image, features_list, DPAM_layer = 20)
        # patch_features is a list aligned with features_list
        # each element: [B, N+1, D] (includes CLS)
        patch_features_out = []
        for feat in patch_features:
            feat = feat / feat.norm(dim=-1, keepdim=True)
            feat = feat[:, 1:]  # remove CLS → [B, HW, D]
            patch_features_out.append(feat)

        return patch_features_out  # list of length 4     
