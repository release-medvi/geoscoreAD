...
def _download(
    ...
    if cache_dir is None:
        raise ValueError("Please set CACHE_DIR environment variable.")
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        ...
        # Use `download_root` if provided, otherwise fallback to your cache

        cache_dir = os.environ.get("CACHE_DIR")
        if cache_dir is None:
            raise ValueError("Please set CACHE_DIR environment variable.")
        cache_dir = download_root or cache_dir
        model_path = _download(_MODELS[name], cache_dir)
