...
class AdaCLIP(nn.Module):
    ...
    ## Added for ZSAD, which computes the foreground / background mask from AdaCLIP visual patch tokens.
    def compute_background_mask_adaclip(
        self,
        patch_tokens,          # List[Tensor[B, HW, C]]
        grid_size,             # (H, W)
        kernel_size=3,
        fg_percentile=90,      # top 10% = foreground
    ):
        """
        Compute foreground / background mask from AdaCLIP visual patch tokens.

        Returns:
            mask: BoolTensor [B, HW]
        """
        device = patch_tokens[0].device
        B, HW, C = patch_tokens[0].shape
        H, W = grid_size

        # ---- aggregate multi-layer visual features ----
        feats = torch.cat(patch_tokens, dim=-1)   # [B, HW, C_total]
        feats = F.normalize(feats, dim=-1)

        masks = []

        for b in range(B):
            x = feats[b].detach().cpu().numpy()    # [HW, C]
            ...
            pc1 = pca.fit_transform(x).squeeze(1)  # [HW]

            abs_pc = np.abs(pc1)
            thresh = np.percentile(abs_pc, fg_percentile)
            mask = abs_pc > thresh                 # [HW]
            ...
            mask = torch.from_numpy(mask.reshape(-1).astype(bool))
            masks.append(mask)

        mask = torch.stack(masks, dim=0).to(device)  # [B, HW]

        # ---- safety fallback ----
        for i in range(B):
            if mask[i].sum() == 0:
                mask[i] = torch.ones_like(mask[i], dtype=torch.bool)
    ...
    def visual_text_similarity(self, image_feature, patch_token, text_feature, aggregation):
        ...
        if aggregation: # in the test stage, we firstly aggregate logits from all hierarchies and then do the softmax normalization
            anomaly_map_logits = torch.mean(torch.stack(anomaly_maps, dim=1), dim=1)
            logit_diff = anomaly_map_logits[:, 1:, :, :] - anomaly_map_logits[:, 0:1, :, :]  # [B, 1, H, W]
            # Min-max normalization per image
            min_val = logit_diff.view(logit_diff.size(0), -1).min(dim=1)[0].view(-1,1,1,1)
            max_val = logit_diff.view(logit_diff.size(0), -1).max(dim=1)[0].view(-1,1,1,1)
            anomaly_map = (logit_diff - min_val) / (max_val - min_val + 1e-8)  # [B,1,H,W], range 0-1
            return anomaly_map, anomaly_map_logits    
        '''
        if aggregation: # in the test stage, we firstly aggregate logits from all hierarchies and then do the softmax normalization
            anomaly_map_logits = torch.mean(torch.stack(anomaly_maps, dim=1), dim=1)
            anomaly_map = torch.softmax(anomaly_map_logits, dim=1)
